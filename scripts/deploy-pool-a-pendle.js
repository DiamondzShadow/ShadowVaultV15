// Pool A v2 — "Blue Chip Pendle" upgrade
// Replaces Aave (1.6% APR) with Pendle PT-gUSDC-25JUN2026 (~5.5% fixed).
// Keeps same Blue Chip basket: WETH 45 / WBTC 35 / USDC 20.
// Uses the same decimal-aware PendleAdapter pattern as Pool B v4 —
// deploy a dedicated adapter instance, deploy new vault+NFT, wire,
// test. Old Pool A at 0x3EABca4E9F1dA0CA6b61a3CC942c09Dd51D77E32 stays
// deployed but gets moved to pools.A_aave_deprecated in deployed.json.
...
async function main() {
  ...
  console.log("\n[1/5] Deploy PendleAdapter (Pool A)");
  ...
  console.log("\n[3/5] Deploy Pool A v2 NFT");
  const NFT = await ethers.getContractFactory("ShadowPositionNFTV15");
  const nft = await NFT.deploy("Blue Chip Pendle", signer.address);
  ...
  tx = await vC.addBasketToken(WETH, 4500, ETH_USD_FEED, 8, 18, 0); await tx.wait(); console.log("  basket WETH 45");
  tx = await vC.addBasketToken(WBTC, 3500, BTC_USD_FEED, 8, 8, 0);  await tx.wait(); console.log("  basket WBTC 35");
  tx = await vC.addBasketToken(USDC, 2000, ZERO_FEED, 0, 6, 0);      await tx.wait(); console.log("  basket USDC 20");
  ...
  if (deployed.pools.A && deployed.pools.A.yieldSource === "aave") {
    deployed.pools.A_aave_deprecated = {
      ...deployed.pools.A,
      note: "v15.2 Blue Chip Aave — superseded by Pool A v2 Pendle 2026-04-11. User's smoke/test positions still live here.",
    };
  }
  deployed.pools.A = {
  ...
  deployed.adapters.pendle_a = adapterAddr;
  fs.writeFileSync(deployedPath, JSON.stringify(deployed, null, 2));
  console.log("basketLength:", (await vC.basketLength()).toString());

  console.log("\n═══ Pool A v2 Pendle deployed ═══");
  console.log("  PendleAdapter (A):", adapterAddr);
  console.log("  Pool A v2 vault:  ", vaultAddr);
  console.log("  Pool A v2 NFT:    ", nftAddr);
}
