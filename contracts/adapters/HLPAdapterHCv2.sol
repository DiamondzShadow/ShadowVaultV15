...
/// @title HLPAdapterHCv2
/// @notice ShadowVaultV15 yield adapter for Hyperliquid HLP — HyperEVM native.
///
///         v2 fixes the spot/perp gap in v1:
///         HLP vaults deposit from / withdraw to the HyperCore **perp** side.
///         `bridgeToCore` lands USDC on **spot**. `sendAsset` (bridgeToEvm)
///         pulls from **spot**. So every round trip needs a
///         `USD_CLASS_TRANSFER` hop. v1 omitted both hops, so deposits stuck
///         on spot and HLP equity never grew.
///
///         Deposit flow (all in one tx, CoreWriter actions are delayed a few
///         seconds and executed in emitted order):
///           1. Pull USDC from vault.
///           2. `bridgeToCore(USDC, amount)`           (EVM → HC spot)
///           3. `transferUsdClass(amount, true)`       (HC spot → HC perp)
///           4. `vaultTransfer(HLP, true, amount)`     (HC perp → HLP)
///           Keeper confirms when precompile equity >= inFlightToHC.
///
///         Withdraw flow (multi-step, HLP lockup on leg 1):
///           1. `initiateHCWithdraw(usd)`   emits vaultTransfer(HLP, false, usd)
///              — HLP → HC perp, gated on precompile lockup timestamp.
///           2. `syncPerpToSpot(usd)`       emits transferUsdClass(usd, false)
///              — HC perp → HC spot. Keeper calls once withdrawable() >= usd.
///           3. `sweepFromCore(usd)`        emits sendAsset to USDC system addr
///              — HC spot → EVM. Keeper calls once spotBalance() >= usd.
///           4. `confirmReturn(amount)`     clears inFlightFromHC counter
///              — keeper calls once adapter EVM USDC balance grew >= usd.
///
///         Accounting:
///           totalAssets() = idle + inFlightToHC + precompile_equity + inFlightFromHC
///
///         Safety:
///           - Per-tx + rolling-24h caps on `deposit`.
///           - USDC cannot be rescued.
///           - `syncAccounting` admin escape hatch for state drift.
///           - `verifyRoute()` admin helper round-trips a tiny USDC before wiring
///             the vault in. v2 round-trips through spot→perp→spot to prove the
///             full lane used by real deposits/withdraws works.
contract HLPAdapterHCv2 is IYieldAdapter, AccessControl, ReentrancyGuard {
    ...
    /// @notice Minimum single-deposit amount. HLP USD accounting is uint64
    ///         6-dec; floor of 1 USDC is a sensible practical limit.
    ...
    uint256 public hlpLockupSecs   = 4 days;          // informational; real lockup enforced by precompile

    // ───────── State — accounting ─────────
    uint256 public totalPrincipal;
    /// @notice USDC bridged to HC + queued for HLP; not yet reflected in precompile equity.
    ...
    event PerpToSpotSynced(uint256 usd6);
    event SpotToPerpSynced(uint256 usd6);
    ...
    ///      Precompile equity = USDC that has successfully landed in HLP.
    ///      inFlightToHC covers the bridge+spot→perp+vaultTransfer gap.
    ///      inFlightFromHC covers the vaultTransfer+perp→spot+sendAsset gap.
        ...
        nonReentrant
    {
        ...
        uint64 usd6 = uint64(amount);

        // 1. EVM → HyperCore spot (uses CoreDepositWallet for USDC).
        CoreWriterLib.bridgeToCore(address(USDC), amount);
        // 2. HC spot → HC perp. HLP takes perp USDC; v1 skipped this and
        //    deposits got stuck on spot.
        CoreWriterLib.transferUsdClass(usd6, true);
        // 3. HC perp → HLP. CoreWriter actions are executed in the order they
        //    are emitted on a given block, so the class transfer lands first.
        CoreWriterLib.vaultTransfer(HLP_VAULT, true, usd6);
    ...
    ///         precompile and is no longer in-flight.
    ...
    ///         Funds land on HC perp after the CoreWriter delay — keeper
    ///         must then call `syncPerpToSpot` before `sweepFromCore`.
    ...
    /// @notice Move USDC from HC perp → HC spot. Call after `initiateHCWithdraw`
    ///         and once `PrecompileLib.withdrawable(adapter) >= usd6`. Required
    ///         before `sweepFromCore`, which uses sendAsset on SPOT_DEX.
    function syncPerpToSpot(uint64 usd6)
        external
        onlyRole(KEEPER_ROLE)
    {
        if (usd6 == 0) revert ZeroAmount();
        CoreWriterLib.transferUsdClass(usd6, false);
        emit PerpToSpotSynced(uint256(usd6));
    }

    /// @notice Admin helper: move USDC from HC spot → HC perp. Useful to
    ///         recover idle HC spot dust into HLP on-ramp, or to pre-position
    ///         before a deferred vaultTransfer. Not on the hot path.
    function syncSpotToPerp(uint64 usd6)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        if (usd6 == 0) revert ZeroAmount();
        CoreWriterLib.transferUsdClass(usd6, true);
        emit SpotToPerpSynced(uint256(usd6));
    }

    /// @notice Keeper bridges HC spot USDC back to the adapter on EVM.
    ///         Call once the perp→spot sync has landed — verify via
    ///         `PrecompileLib.spotBalance(adapter, USDC).total >= usd6`.
    /// @dev Requires the adapter to hold a small HYPE float on HC to pay the
    ///      sendAsset gas (~200k gas at HyperEVM base price per docs). Keeper
    ///      or admin must top up HYPE periodically.
    ...
    /// @notice Admin-only deploy-time route test. Bridges `amount` USDC
    ///         EVM→spot→perp→spot→EVM without touching HLP, proving every
    ///         hop in the real flow works. Run ONCE before VAULT_ROLE grant.
    ...
    /// @notice USDC sitting on the adapter's HC perp side (pre-spot sync).
    function reportedPerpUsdc() external view returns (uint64) {
        return PrecompileLib.withdrawable(address(this));
    }

    /// @notice USDC sitting on the adapter's HC spot side (pre-EVM sweep).
    function reportedSpotUsdc() external view returns (uint64) {
        return PrecompileLib.spotBalance(address(this), address(USDC)).total;
    }
